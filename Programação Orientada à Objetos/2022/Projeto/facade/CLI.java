...
import facade.aux.ResearchHelper;
import facade.aux.Tuple;
...
public class CLI {
	...
	public static void auto(boolean skipPauses) {
		...
		Menu[] juneMenus = {
				new Menu("Milho", 4.0),
				new Menu("Pipoca", 5.0),
				new Menu("Canjica", 8.0),
				new Menu("Pamonha", 10.0),
				new Menu("Pé de moleque", 6.0,
						"Feito de amendoim e milho, não é de um moleque de verdade"),
		};
		Restaurant restaurant = new Restaurant("El Baratinho",
				"Litoral, Pacífico Leste, Rua: Azulão, Número: 17-q Bairro: Só", menus);
		...
		Collection<Menu> itens = restaurant.getMenu().getItems();
		...
		for (Restaurant _restaurant : restaurants) {
			System.out.printf("%22s | %s",
					_restaurant.getName(), _restaurant.getAddress());
		}
		...
		if (demands != null)
			for (Demand _demand : demands) {
				Menu item = restaurant.getMenu().getItem(_demand.getId());

				if (item != null)
					System.out.printf("Pedido <%s>: %s.\n", _demand.getId(), item.toString());
			}

		/*
		 * O restaurante irá criar um cardápio específico para as festas juninas.
		 * Cadastre esse novo cardápio associado a um determinado restaurante.
		 */
		restaurant = restaurants[restaurants.length - 1];

		pauseLog(skipPauses, String.format(
				"O restaurante %s está adotando um cardápio junino:", restaurant.toString()));

		for (Menu item : juneMenus) {
			restaurant.getMenu().addMenu(item);
			System.out.printf("Item <%s> adicionado ao cardápio", item.toString());
		}

		/*
		 * Pesquisa dos usuários que mais realizaram pedidos.
		 */
		ResearchHelper rh = new ResearchHelper(restaurants[0], restaurant);
		// Pessoa física
		Tuple mostDemands = rh.process(individuals);
		System.out.printf("A \"pessoa física\" com mais pedidos (%d) é: %s.\n",
				mostDemands.nDemands, mostDemands.person.toString());
		// Pessoa jurídica
		mostDemands = rh.process(legalPersons);
		System.out.printf("A pessoa jurídica com mais pedidos (%d) é: %s.\n",
				mostDemands.nDemands, mostDemands.person.toString());
		// Total
		System.out.printf("A pessoa física com mais pedidos no geral (%d) é: %s.\n",
				rh._total.nDemands, rh._total.person.toString());
	}

	private static void pauseLog(boolean skipPauses, String message) {
		System.out.println();
	...
	// private static void pause(boolean skipPauses) {
	// System.out.println();
	// if (skipPauses)
	// return;
	// System.out.println("Pressione Enter para continuar...");
	...
	private static Person promptNewPerson(Scanner scanner) {
		...
		System.out.println("Insert person cpf: ");
		cpf = scanner.nextLine();

		// WATCH -> Permitir retornar uma pessoa jurídica.
		return new Individual(name, cpf, LocalDate.of(birthYear, birthMonth, birthDay));
	}
	...
	private static Restaurant promptNewRestaurant(
		...
		return new Restaurant(name, address, (Menu[]) defaultMenu.values().toArray());
	}
